...
const RepoTemplate = `
<svg width="540" viewBox="0 0 660 {{.Height}}" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-labelledby="descId">
    <title id="titleId">{{.Repo}}'s Code Stats</title>
    <desc id="descId">Repository Code Statistics</desc>
    <style>.oct-icon{display:block;fill:#8b849e;}.repo-color{animation:fadeInAnimation 0.8s ease-in-out forwards;}.header{font:600 18px 'Segoe UI',Ubuntu,Sans-Serif;fill:#2f80ed;animation:fadeInAnimation 0.8s ease-in-out forwards;}@supports(appearance:auto){.header{font-size:15.5px;}}.stat{font:600 14px 'Segoe UI',Ubuntu,"Helvetica Neue",Sans-Serif;fill:{{if .Dark}}"#fff"{{else}}"#434d58"{{end}};}@supports(appearance:auto){.stat{font-size:12px;}}.stagger{opacity:0;animation:fadeInAnimation 0.3s ease-in-out forwards;}.bold{font-weight:700}.icon{fill:#4c71f2;display:none;}@keyframes growWidthAnimation{from{width:0;}to{width:100%;}}.lang-name{font:400 11px "Segoe UI",Ubuntu,Sans-Serif;fill:{{if .Dark}}"#fff"{{else}}"#434d58"{{end}};}.stagger{opacity:0;animation:fadeInAnimation 0.3s ease-in-out forwards;}#rect-mask rect{animation:fadeInAnimation 1s ease-in-out forwards;}@keyframes scaleInAnimation{from{transform:translate(-5px,5px) scale(0);}to{transform:translate(-5px,5px) scale(1);}}@keyframes rankAnimation{from{stroke-dashoffset:251.32741228718345;}to{stroke-dashoffset:125.45405959298056;}}@keyframes scaleInAnimation{from{transform:translate(-5px,5px) scale(0);}to{transform:translate(-5px,5px) scale(1);}}@keyframes fadeInAnimation{from{opacity:0;}to{opacity:1;}}</style>
    <rect data-testid="card-bg" x="0.5" y="0.5" rx="4.5" height="99%" stroke="#e4e2e2" width="659" fill="{{if .Dark}}"#000"{{else}}"#fffefe"{{end}}" stroke-opacity="1"/>
    ...
    <g xmlns="http://www.w3.org/2000/svg" transform="translate(300, 55)" data-testid="main-card-progress">
        ...
        <svg data-testid="lang-items" x="25">
            ...
            <g transform="translate(0, 25)">
                <g transform="translate(0, 0)">
                    {{- range $index, $lang := (slice .Langs 0 $.Left) }}
